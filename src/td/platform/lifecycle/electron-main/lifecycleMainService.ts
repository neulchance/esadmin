...
import {Barrier, Promises, timeout} from 'td/base/common/async';
...
import {IDevWindow, LoadReason, UnloadReason} from 'td/platform/window/electron-main/window';
...
export interface ILifecycleMainService {
	 ...
	 */
	readonly onBeforeCloseWindow: Event<IDevWindow>;

	/**
	 * Make a `IDevWindow` known to the lifecycle main service.
	 */
	registerWindow(window: IDevWindow): void;
	 ...
	 */
	reload(window: IDevWindow, cli?: NativeParsedArgs): Promise<void>;

	/**
	 * Unload a window for the provided reason. All lifecycle event handlers are triggered.
	 */
	unload(window: IDevWindow, reason: UnloadReason): Promise<boolean /* veto */>;
...
export class LifecycleMainService extends Disposable implements ILifecycleMainService {
	...
	private readonly _onBeforeCloseWindow = this._register(new Emitter<IDevWindow>());
	...
	async reload(window: IDevWindow, cli?: NativeParsedArgs): Promise<void> {
	...
	private async doUnload(window: IDevWindow, reason: UnloadReason): Promise<boolean /* veto */> {
	...
	private onBeforeUnloadWindowInRenderer(window: IDevWindow, reason: UnloadReason): Promise<boolean /* veto */> {
	...
	private onWillUnloadWindowInRenderer(window: IDevWindow, reason: UnloadReason): Promise<void> {
