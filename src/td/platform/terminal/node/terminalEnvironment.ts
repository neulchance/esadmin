...
import {getCaseInsensitive} from 'td/base/common/objects';
import * as path from 'td/base/common/path';
import {IProcessEnvironment, isMacintosh, isWindows} from 'td/base/common/platform';
import * as process from 'td/base/common/process';
import {format} from 'td/base/common/strings';
...
import {IShellLaunchConfig, ITerminalEnvironment, ITerminalProcessOptions} from 'td/platform/terminal/common/terminal';
import {EnvironmentVariableMutatorType} from 'td/platform/terminal/common/environmentVariable';
...
export function getShellIntegrationInjection(
	...
	switch (shell) {
		...
		case 'fish': {
			...
			const newDataDir = path.join(appRoot, 'out/td/workbench/contrib/terminal/browser/media/fish_xdg_data');
		...
		case 'zsh': {
			...
			filesToCopy.push({
				source: path.join(appRoot, 'out/td/workbench/contrib/terminal/browser/media/shellIntegration-rc.zsh'),
				dest: path.join(zdotdir, '.zshrc')
			});
			filesToCopy.push({
				source: path.join(appRoot, 'out/td/workbench/contrib/terminal/browser/media/shellIntegration-profile.zsh'),
				dest: path.join(zdotdir, '.zprofile')
			});
			filesToCopy.push({
				source: path.join(appRoot, 'out/td/workbench/contrib/terminal/browser/media/shellIntegration-env.zsh'),
				dest: path.join(zdotdir, '.zshenv')
			});
			filesToCopy.push({
				source: path.join(appRoot, 'out/td/workbench/contrib/terminal/browser/media/shellIntegration-login.zsh'),
...
shellIntegrationArgs.set(ShellIntegrationExecutable.Pwsh, ['-noexit', '-command', '. "{0}/out/td/workbench/contrib/terminal/browser/media/shellIntegration.ps1"{1}']);
shellIntegrationArgs.set(ShellIntegrationExecutable.PwshLogin, ['-l', '-noexit', '-command', '. "{0}/out/td/workbench/contrib/terminal/browser/media/shellIntegration.ps1"']);
shellIntegrationArgs.set(ShellIntegrationExecutable.Zsh, ['-i']);
shellIntegrationArgs.set(ShellIntegrationExecutable.ZshLogin, ['-il']);
shellIntegrationArgs.set(ShellIntegrationExecutable.Bash, ['--init-file', '{0}/out/td/workbench/contrib/terminal/browser/media/shellIntegration-bash.sh']);
