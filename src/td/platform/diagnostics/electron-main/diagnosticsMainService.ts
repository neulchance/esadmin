...
import {IDiagnosticInfo, IDiagnosticInfoOptions, IMainProcessDiagnostics, IProcessDiagnostics, IRemoteDiagnosticError, IRemoteDiagnosticInfo, IWindowDiagnostics} from 'td/platform/diagnostics/common/diagnostics';
...
export class DiagnosticsMainService implements IDiagnosticsMainService {
	...
	private async codeWindowToInfo(window: IDevWindow): Promise<IWindowDiagnostics> {
	...
	private async getFolderURIs(window: IDevWindow): Promise<URI[]> {
		...
		if (isSingleFolderWorkspaceIdentifier(workspace)) {
			...
			if (resolvedWorkspace) {
				...
				rootFolders.forEach((root: { uri: URI; }) => {
