...
import {sanitizeProcessEnvironment} from 'td/base/common/processes';
import * as pfs from 'td/base/node/pfs';
import * as processes from 'td/base/node/processes';
import * as nls from 'td/nls';
import {DEFAULT_TERMINAL_OSX, IExternalTerminalService, IExternalTerminalSettings, ITerminalForPlatform} from 'td/platform/externalTerminal/common/externalTerminal';
import {ITerminalEnvironment} from 'td/platform/terminal/common/terminal';
	...
	public runInTerminal(title: string, dir: string, args: string[], envVars: ITerminalEnvironment, settings: IExternalTerminalSettings): Promise<number | undefined> {
		...
		return new Promise<number | undefined>((resolve, reject) => {
			...
			if (terminalApp === DEFAULT_TERMINAL_OSX || terminalApp === 'iTerm.app') {
				...
				const scriptpath = FileAccess.asFileUri(`td/workbench/contrib/externalTerminal/node/${script}.scpt`).fsPath;
