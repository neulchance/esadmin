...
import {addUNCHostToAllowlist} from 'td/base/node/unc';
...
import {isWindowsDriveLetter, parseLineAndColumnAware, sanitizeFilePath, toSlashes} from 'td/base/common/extpath';
...
import {basename, join, normalize, posix} from 'td/base/common/path';
import {getMarks, mark} from 'td/base/common/performance';
import {IProcessEnvironment, isMacintosh, isWindows, OS} from 'td/base/common/platform';
import {cwd} from 'td/base/common/process';
import {extUriBiasedIgnorePathCase, isEqualAuthority, normalizePath, originalFSPath, removeTrailingPathSeparator} from 'td/base/common/resources';
...
import {FileType, IFileService} from 'td/platform/files/common/files';
...
import {IAddFoldersRequest, INativeOpenFileRequest, INativeWindowConfiguration, IOpenEmptyWindowOptions, IPath, IPathsToWaitFor, isFileToOpen, isFolderToOpen, isWorkspaceToOpen, IWindowOpenable, IWindowSettings} from 'td/platform/window/common/window';
import {DevWindow} from 'td/platform/windows/electron-main/windowImpl';
import {IOpenConfiguration, IOpenEmptyConfiguration, IWindowsCountChangedEvent, IWindowsMainService, OpenContext, getLastFocused} from 'td/platform/windows/electron-main/windows';
import {findWindowOnExtensionDevelopmentPath, findWindowOnFile, findWindowOnWorkspaceOrFolder} from 'td/platform/windows/electron-main/windowsFinder';
import {IWindowState, WindowsStateHandler} from 'td/platform/windows/electron-main/windowsStateHandler';
import {IRecent} from 'td/platform/workspaces/common/workspaces';
import {hasWorkspaceFileExtension, IAnyWorkspaceIdentifier, ISingleFolderWorkspaceIdentifier, isSingleFolderWorkspaceIdentifier, isWorkspaceIdentifier, IWorkspaceIdentifier, toWorkspaceIdentifier} from 'td/platform/workspace/common/workspace';
import {createEmptyWorkspaceIdentifier, getSingleFolderWorkspaceIdentifier, getWorkspaceIdentifier} from 'td/platform/workspaces/node/workspaces';
...
import {IDevWindow, UnloadReason} from 'td/platform/window/electron-main/window';
import {IThemeMainService} from 'td/platform/theme/electron-main/themeMainService';
import {IEditorOptions, ITextEditorOptions} from 'td/platform/editor/common/editor';
...
export class WindowsMainService extends Disposable implements IWindowsMainService {
	...
	private readonly _onDidChangeFullScreen = this._register(new Emitter<{ window: IDevWindow; fullscreen: boolean }>());
	...
	private handleWaitMarkerFile(openConfig: IOpenConfiguration, usedWindows: IDevWindow[]): void {
	...
	private async doOpen(
	...
	): Promise<{ windows: IDevWindow[]; filesOpenedInWindow: IDevWindow | undefined }> {

		// Keep track of used windows and remember
		// if files have been opened in one of them
		const usedWindows: IDevWindow[] = [];
		let filesOpenedInWindow: IDevWindow | undefined = undefined;
		...
		if (filesToOpen && potentialNewWindowsCount === 0) {
			...
			// this step.
			let windowToUseForFiles: IDevWindow | undefined = undefined;
	...
	private doOpenFilesInExistingWindow(configuration: IOpenConfiguration, window: IDevWindow, filesToOpen?: IFilesToOpen): IDevWindow {
	...
	private focusMainOrChildWindow(mainWindow: IDevWindow): void {
		let windowToFocus: IDevWindow | IAuxiliaryWindow = mainWindow;
	...
	private doAddFoldersToExistingWindow(window: IDevWindow, foldersToAdd: URI[]): IDevWindow {
	...
	private doOpenEmpty(openConfig: IOpenConfiguration, forceNewWindow: boolean, remoteAuthority: string | undefined, filesToOpen: IFilesToOpen | undefined, emptyWindowBackupInfo?: IEmptyWindowBackupInfo): Promise<IDevWindow> {
		...
		let windowToUse: IDevWindow | undefined;
	...
	private doOpenFolderOrWorkspace(openConfig: IOpenConfiguration, folderOrWorkspace: IWorkspacePathToOpen | ISingleFolderWorkspacePathToOpen, forceNewWindow: boolean, filesToOpen: IFilesToOpen | undefined, windowToUse?: IDevWindow): Promise<IDevWindow> {
	...
	async openExtensionDevelopmentHostWindow(extensionDevelopmentPaths: string[], openConfig: IOpenConfiguration): Promise<IDevWindow[]> {
	...
	private async doOpenInBrowserWindow(window: IDevWindow, configuration: INativeWindowConfiguration, options: IOpenBrowserWindowOptions, defaultProfile: IUserDataProfile): Promise<void> {
	...
	private onWindowClosed(window: IDevWindow, disposables: IDisposable): void {
	...
	private onWindowDestroyed(window: IDevWindow): void {
