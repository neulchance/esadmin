...
import {observableFromEvent, derived} from 'td/base/common/observable';
...
export class AudioCueService extends Disposable implements IAudioCueService {
	...
	public async playSound(sound: Sound, allowManyInParallel = false): Promise<void> {
		...
		const url = FileAccess.asBrowserUri(`td/platform/audioCues/browser/media/${sound.fileName}`).toString(true);
