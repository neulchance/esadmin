...
(function (global, factory) {
  ...
  let getEscapeReplacement = function getEscapeReplacement(ch) {
  ...
  let unescapeTest = /&(#(?:\d+)|(?:#x[0-9A-Fa-f]+)|(?:\w+));?/ig;
  /**
   * @param {string} html
   */
  ...
  function indentCodeCompensation(raw, text) {
    ...
    return text.split('\n').map(function (node) {
      ...
      let indentInNode = matchIndentInNode[0];
  ...
  let Tokenizer = /*#__PURE__*/function () {
    function Tokenizer(options) {
      this.options = options || exports.defaults;
    }

    let _proto = Tokenizer.prototype;
      ...
      if (cap) {
        ...
        while (src) {
          ...
          if (!endEarly) {
            ...
            let hrRegex = new RegExp("^ {0," + Math.min(3, indent - 1) + "}((?:- *){3,}|(?:_ *){3,}|(?:\\* *){3,})(?:\\n+|$)");
            let fencesBeginRegex = new RegExp("^ {0," + Math.min(3, indent - 1) + "}(?:```|~~~)");
            let headingBeginRegex = new RegExp("^ {0," + Math.min(3, indent - 1) + "}#"); // Check if following lines should be included in List Item
        ...
        let l = list.items.length; // Item child tokens handled here at end because we needed to have the final item to trim it first

        for (i = 0; i < l; i++) {
          this.lexer.state.top = false;
          list.items[i].tokens = this.lexer.blockTokens(list.items[i].text, []);
          let spacers = list.items[i].tokens.filter(function (t) {
            return t.type === 'space';
          });
          let hasMultipleLineBreaks = spacers.every(function (t) {
            let chars = t.raw.split('');
            let lineBreaks = 0;

            for (var _iterator = _createForOfIteratorHelperLoose(chars), _step; !(_step = _iterator()).done;) {
              let _char = _step.value;
    ...
    _proto.link = function link(src) {
      ...
      if (cap) {
        ...
        if (!this.options.pedantic && /^</.test(trimmedUrl)) {
          ...
          if (lastParenIndex > -1) {
            let start = cap[0].indexOf('!') === 0 ? 5 : 4;
            let linkLen = start + cap[1].length + lastParenIndex;
    ...
    _proto.emStrong = function emStrong(src, maskedSrc, prevChar) {
      ...
      if (!nextChar || nextChar && (prevChar === '' || this.rules.inline.punctuation.exec(prevChar))) {
        ...
        let rDelim,
            ...
            delimTotal = lLength,
            midDelimTotal = 0;
        let endReg = match[0][0] === '*' ? this.rules.inline.emStrong.rDelimAst : this.rules.inline.emStrong.rDelimUnd;
        ...
        while ((match = endReg.exec(maskedSrc)) != null) {
          ...
          if (Math.min(lLength, rLength) % 2) {
            let _text = src.slice(1, lLength + match.index + rLength);
          ...
          let text = src.slice(2, lLength + match.index + rLength - 1);
          return {
            type: 'strong',
            raw: src.slice(0, lLength + match.index + rLength + 1),
            text: text,
    ...
    _proto.codespan = function codespan(src) {
      ...
      if (cap) {
        ...
        let hasNonSpaceChars = /[^ ]/.test(text);
        let hasSpaceCharsOnBothEnds = /^ /.test(text) && / $/.test(text);
      ...
      while (src) {
        ...
        if (this.options.extensions && this.options.extensions.startBlock) {
          (function () {
            ...
            let tempSrc = src.slice(1);
            let tempStart = void 0;
  ...
  let Renderer = /*#__PURE__*/function () {
    ...
    let _proto = Renderer.prototype;

    _proto.code = function code(_code, infostring, escaped) {
      let lang = (infostring || '').match(/\S*/)[0];

      if (this.options.highlight) {
        let out = this.options.highlight(_code, lang);
   ...
   */
  let TextRenderer = /*#__PURE__*/function () {
    function TextRenderer() {}

    let _proto = TextRenderer.prototype;
  ...
  marked.use = function () {
    ...
    let opts = merge.apply(void 0, [{}].concat(args));
    ...
    args.forEach(function (pack) {
      ...
      if (pack.extensions) {
        ...
        pack.extensions.forEach(function (ext) {
          ...
          if (ext.renderer) {
            // Renderer extensions
            let prevRenderer = extensions.renderers ? extensions.renderers[ext.name] : null;
      ...
      if (pack.renderer) {
        (function () {
          ...
          let _loop = function _loop(prop) {
            let prevRenderer = renderer[prop]; // Replace renderer with func to run extension, but fall back if false
      ...
      if (pack.tokenizer) {
        (function () {
          ...
          let _loop2 = function _loop2(prop) {
            let prevTokenizer = tokenizer[prop]; // Replace tokenizer with func to run extension, but fall back if false
      ...
      if (pack.walkTokens) {
        let _walkTokens = marked.defaults.walkTokens;
  ...
  marked.walkTokens = function (tokens, callback) {
    ...
    let _loop3 = function _loop3() {
      let token = _step.value;
        ...
        case 'table':
          {
            for (var _iterator2 = _createForOfIteratorHelperLoose(token.header), _step2; !(_step2 = _iterator2()).done;) {
              let cell = _step2.value;
              values = values.concat(marked.walkTokens(cell.tokens, callback));
            }

            for (var _iterator3 = _createForOfIteratorHelperLoose(token.rows), _step3; !(_step3 = _iterator3()).done;) {
              let row = _step3.value;

              for (var _iterator4 = _createForOfIteratorHelperLoose(row), _step4; !(_step4 = _iterator4()).done;) {
                let _cell = _step4.value;
