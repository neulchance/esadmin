	...
	return new Promise((resolve, reject) => {
		...
		if (process.platform === 'win32') {
			...
			(import('@vscode/windows-process-tree')).then(windowsProcessTree => {
				windowsProcessTree.getProcessList(rootPid, (processList: any) => {
					if (!processList) {
					...
					windowsProcessTree.getProcessCpuUsage(processList, (completeProcessList: any) => {
						...
						completeProcessList.forEach((process: { commandLine: any; pid: number; ppid: any; cpu: any; memory: any; }) => {
			...
			function calculateLinuxCpuUsage() {
				...
				let cmd = JSON.stringify(FileAccess.asFileUri('td/base/node/cpuUsage.sh').fsPath);
				cmd += ' ' + pids.join(' ');
			...
			exec('which ps', {}, (err, stdout, stderr) => {
				if (err || stderr) {
					if (process.platform !== 'linux') {
						...
						const cmd = JSON.stringify(FileAccess.asFileUri('td/base/node/ps.sh').fsPath);
						exec(cmd, {}, (err, stdout, stderr) => {
							if (err || stderr) {
								reject(err || new Error(stderr.toString()));
							} else {
