...
import {IWorker, IWorkerCallback, IWorkerFactory, logOnceWebWorkerWarning} from 'td/base/common/worker/simpleWorker';
...
function getWorker(label: string): Worker | Promise<Worker> {
	...
	if (typeof require === 'function') {
		...
		const workerMain = require.toUrl('td/base/worker/workerMain.js'); // explicitly using require.toUrl(), see https://github.com/microsoft/vscode/issues/107440#issuecomment-698982321
...
export function getWorkerBootstrapUrl(scriptPath: string, label: string): string {
	if (/^((http:)|(https:)|(file:))/.test(scriptPath) && scriptPath.substring(0, globalThis.origin.length) !== globalThis.origin) {
		...
		const myPath = 'td/base/worker/defaultWorkerFactory.js';
