...
import {AbstractIdleValue, IntervalTimer, TimeoutTimer, _runWhenIdle, IdleDeadline} from 'td/base/common/async';
import {onUnexpectedError} from 'td/base/common/errors';
import * as event from 'td/base/common/event';
import * as dompurify from 'td/base/browser/dompurify/dompurify';
...
import {FileAccess, RemoteAuthorities, Schemas} from 'td/base/common/network';
...
export const {
	...
	return {
		...
		registerWindow(window: DevWindow): IDisposable {
			if (windows.has(window.tddevWindowId)) {
			...
			windows.set(window.tddevWindowId, registeredWindow);

			disposables.add(toDisposable(() => {
				windows.delete(window.tddevWindowId);
		...
		getWindowId(targetWindow: Window): number {
			return (targetWindow as DevWindow).tddevWindowId;
		},
		...
		getWindow(e: Node | UIEvent | undefined | null): DevWindow {
			...
			if (candidateNode?.ownerDocument?.defaultView) {
				return candidateNode.ownerDocument.defaultView.window as DevWindow;
			}

			const candidateEvent = e as UIEvent | undefined | null;
			if (candidateEvent?.view) {
				return candidateEvent.view.window as DevWindow;
			}
 ...
 * the main window.
 */
export function getActiveWindow(): DevWindow {
	const document = getActiveDocument();
	return (document.defaultView?.window ?? mainWindow) as DevWindow;
}
