...
import {isESM} from 'td/base/common/amd';
import {AppResourcePath, FileAccess, nodeModulesAsarPath, nodeModulesPath} from 'td/base/common/network';
import * as platform from 'td/base/common/platform';
import {IProductConfiguration} from 'td/base/common/product';
...
export async function importAMDNodeModule<T>(nodeModuleName: string, pathInsideNodeModule: string, isBuilt?: boolean): Promise<T> {
	if (isESM) {
		...
		if (/^\w[\w\d+.-]*:\/\//.test(nodeModulePath)) {
			// looks like a URL
			// bit of a special case for: src/td/workbench/services/languageDetection/browser/languageDetectionSimpleWorker.ts
