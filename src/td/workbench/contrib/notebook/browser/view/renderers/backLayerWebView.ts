...
import {WorkbenchActionExecutedClassification, WorkbenchActionExecutedEvent} from 'td/base/common/actions';
import {coalesce} from 'td/base/common/arrays';
import {DeferredPromise, runWhenGlobalIdle} from 'td/base/common/async';
import {decodeBase64} from 'td/base/common/buffer';
import {Emitter, Event} from 'td/base/common/event';
import {IDisposable} from 'td/base/common/lifecycle';
import {getExtensionForMimeType} from 'td/base/common/mime';
import {FileAccess, Schemas, matchesScheme, matchesSomeScheme} from 'td/base/common/network';
import {equals} from 'td/base/common/objects';
import * as osPath from 'td/base/common/path';
import {isMacintosh, isWeb} from 'td/base/common/platform';
import {dirname, extname, isEqual, joinPath} from 'td/base/common/resources';
import {URI} from 'td/base/common/uri';
import * as UUID from 'td/base/common/uuid';
...
import {IFileDialogService} from 'td/platform/dialogs/common/dialogs';
...
import {editorFindMatch, editorFindMatchHighlight} from 'td/platform/theme/common/colorRegistry';
...
import {CellEditState, ICellOutputViewModel, ICellViewModel, ICommonCellInfo, IDisplayOutputLayoutUpdateRequest, IDisplayOutputViewModel, IFocusNotebookCellOptions, IGenericCellViewModel, IInsetRenderOutput, INotebookEditorCreationOptions, INotebookWebviewMessage, RenderOutputType} from 'td/workbench/contrib/notebook/browser/notebookBrowser';
import {NOTEBOOK_WEBVIEW_BOUNDARY} from 'td/workbench/contrib/notebook/browser/view/notebookCellList';
import {preloadsScriptStr} from 'td/workbench/contrib/notebook/browser/view/renderers/webviewPreloads';
import {transformWebviewThemeVars} from 'td/workbench/contrib/notebook/browser/view/renderers/webviewThemeMapping';
import {MarkupCellViewModel} from 'td/workbench/contrib/notebook/browser/viewModel/markupCellViewModel';
import {CellUri, ICellOutput, INotebookRendererInfo, RendererMessagingSpec} from 'td/workbench/contrib/notebook/common/notebookCommon';
import {INotebookKernel} from 'td/workbench/contrib/notebook/common/notebookKernelService';
import {INotebookLoggingService} from 'td/workbench/contrib/notebook/common/notebookLoggingService';
import {IScopedRendererMessaging} from 'td/workbench/contrib/notebook/common/notebookRendererMessagingService';
import {INotebookService} from 'td/workbench/contrib/notebook/common/notebookService';
import {IWebviewElement, IWebviewService, WebviewContentPurpose, WebviewOriginStore} from 'td/workbench/contrib/webview/browser/webview';
import {WebviewWindowDragMonitor} from 'td/workbench/contrib/webview/browser/webviewWindowDragMonitor';
import {asWebviewUri, webviewGenericCspSource} from 'td/workbench/contrib/webview/common/webview';
...
export class BackLayerWebView<T extends ICommonCellInfo> extends Themable {
	...
	createWebview(codeWindow: DevWindow): Promise<void> {
		const baseUrl = this.asWebviewUri(this.getNotebookBaseUri(), undefined);
		const htmlContent = this.generateContent(baseUrl.toString());
	...
	private getBuiltinLocalResourceRoots(): URI[] {
		...
		return [
			dirname(FileAccess.asFileUri('td/loader.js')),
		];
	}

	private _initialize(content: string, codeWindow: DevWindow): Promise<void> {
	...
	private _createInset(webviewService: IWebviewService, content: string, codeWindow: DevWindow) {
