...
import {EXTENSION_INSTALL_DEP_PACK_CONTEXT, EXTENSION_INSTALL_SKIP_WALKTHROUGH_CONTEXT, IExtensionManagementService} from 'td/platform/extensionManagement/common/extensionManagement';
import {ThemeIcon} from 'td/base/common/themables';
import {walkthroughs} from 'td/workbench/contrib/welcomeGettingStarted/common/gettingStartedContent';
...
import {ILink, LinkedText, parseLinkedText} from 'td/base/common/linkedText';
import {walkthroughsExtensionPoint} from 'td/workbench/contrib/welcomeGettingStarted/browser/gettingStartedExtensionPoint';
import {InstantiationType, registerSingleton} from 'td/platform/instantiation/common/extensions';
import {dirname} from 'td/base/common/path';
import {coalesce, flatten} from 'td/base/common/arrays';
...
import {checkGlobFileExists} from 'td/workbench/services/extensions/common/workspaceContains';
...
import {DefaultIconPath} from 'td/workbench/services/extensionManagement/common/extensionManagement';
							...
							media: step.media.type === 'image'
								...
								: step.media.type === 'svg'
									? {
										type: 'svg',
										altText: step.media.altText,
										path: convertInternalMediaPathToFileURI(step.media.path).with({query: JSON.stringify({moduleId: 'td/workbench/contrib/welcomeGettingStarted/common/media/' + step.media.path})})
									}
									: {
										type: 'markdown',
										path: convertInternalMediaPathToFileURI(step.media.path).with({query: JSON.stringify({moduleId: 'td/workbench/contrib/welcomeGettingStarted/common/media/' + step.media.path})}),
										base: FileAccess.asFileUri('td/workbench/contrib/welcomeGettingStarted/common/media/'),
										root: FileAccess.asFileUri('td/workbench/contrib/welcomeGettingStarted/common/media/'),
									},
						});
					})
			});
...
export const convertInternalMediaPathToFileURI = (path: string) => path.startsWith('https://')
	? URI.parse(path, true)
	: FileAccess.asFileUri(`td/workbench/contrib/welcomeGettingStarted/common/media/${path}`);

const convertInternalMediaPathToBrowserURI = (path: string) => path.startsWith('https://')
	? URI.parse(path, true)
	: FileAccess.asBrowserUri(`td/workbench/contrib/welcomeGettingStarted/common/media/${path}`);
