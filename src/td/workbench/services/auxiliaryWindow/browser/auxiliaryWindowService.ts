...
import {IRectangle, WindowMinimumSize} from 'td/platform/window/common/window';
...
export class BrowserAuxiliaryWindowService extends Disposable implements IAuxiliaryWindowService {
	...
	async open(options?: IAuxiliaryWindowOpenOptions): Promise<IAuxiliaryWindow> {
		...
		// Add a `vsDevWindowId` property to identify auxiliary windows
		const resolvedWindowId = await this.resolveWindowId(targetWindow);
		ensureDevWindow(targetWindow, resolvedWindowId);
		...
		this.windows.set(targetWindow.tddevWindowId, auxiliaryWindow);
		registryDisposables.add(toDisposable(() => this.windows.delete(targetWindow.tddevWindowId)));
	...
	protected createAuxiliaryWindow(targetWindow: DevWindow, container: HTMLElement, stylesLoaded: Barrier): AuxiliaryWindow {
	...
	protected createContainer(auxiliaryWindow: DevWindow, disposables: DisposableStore, options?: IAuxiliaryWindowOpenOptions): { stylesLoaded: Barrier; container: HTMLElement } {
	...
	protected patchMethods(auxiliaryWindow: DevWindow): void {
	...
	private applyMeta(auxiliaryWindow: DevWindow): void {
	...
	protected applyCSS(auxiliaryWindow: DevWindow, disposables: DisposableStore) {
	...
	private applyHTML(auxiliaryWindow: DevWindow, disposables: DisposableStore): HTMLElement {
