...
import {IWindowOpenable, IOpenWindowOptions, isFolderToOpen, isWorkspaceToOpen, IOpenEmptyWindowOptions, IPoint, IRectangle} from 'td/platform/window/common/window';
...
import {disposableWindowInterval, getActiveDocument, getWindowId, getWindowsCount, hasWindow, onDidRegisterWindow} from 'td/base/browser/dom';
import {memoize} from 'td/base/common/decorators';
import {isAuxiliaryWindow} from 'td/base/browser/window';
...
class WorkbenchHostService extends Disposable implements IHostService {
	...
	get onDidChangeActiveWindow(): Event<number> {
		...
		this._register(onDidRegisterWindow(({window, disposables}) => {
			...
			disposables.add(disposableWindowInterval(window, () => {
				...
				if (hasFocus) {
					emitter.fire(window.tddevWindowId);
				}
	...
	toggleFullScreen(targetWindow: Window): Promise<void> {
		return this.nativeHostService.toggleFullScreen({targetWindowId: isAuxiliaryWindow(targetWindow) ? targetWindow.tddevWindowId : undefined});
	}
	...
	async moveTop(targetWindow: Window): Promise<void> {
		...
		return this.nativeHostService.moveWindowTop(isAuxiliaryWindow(targetWindow) ? {targetWindowId: targetWindow.tddevWindowId} : undefined);
	}
