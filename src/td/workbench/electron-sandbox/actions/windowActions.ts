...
import 'td/css!./media/actions';
import {URI} from 'td/base/common/uri';
import {localize, localize2} from 'td/nls';
import {ApplyZoomTarget, MAX_ZOOM_LEVEL, MIN_ZOOM_LEVEL, applyZoom} from 'td/platform/window/electron-sandbox/window';
...
import {IQuickInputService, IQuickInputButton, IQuickPickItem} from 'td/platform/quickinput/common/quickInput';
...
import {isSingleFolderWorkspaceIdentifier, isWorkspaceIdentifier} from 'td/platform/workspace/common/workspace';
...
import {IOpenedAuxiliaryWindow, IOpenedMainWindow, isOpenedAuxiliaryWindow} from 'td/platform/window/common/window';
...
export class CloseWindowAction extends Action2 {
	...
	override async run(accessor: ServicesAccessor): Promise<void> {
		...
		return nativeHostService.closeWindow({targetWindowId: getActiveWindow().tddevWindowId});
	}
}
...
abstract class BaseSwitchWindow extends Action2 {
	...
	override async run(accessor: ServicesAccessor): Promise<void> {
		...
		const currentWindowId = getActiveWindow().tddevWindowId;
